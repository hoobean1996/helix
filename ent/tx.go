...
func (tx *Tx) init() {
	tx.EntEmail = NewEntEmailClient(tx.config)
	tx.EntTemporaryEmail = NewEntTemporaryEmailClient(tx.config)
	tx.EntUser = NewEntUserClient(tx.config)
}
...
// applies a query, for example: EntEmail.QueryXXX(), the query will be executed
